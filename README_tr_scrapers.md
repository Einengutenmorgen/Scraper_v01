...
**Per-article resilience (full-scale harvests).** A single problematic article
(a transient HTTP 405/redirect bounce, an empty/stub body) is RECORDED under
`skipped` (id, url, reason) and skipped — it does NOT abort a multi-thousand
harvest. This is loud-but-not-fatal: the count and reasons are printed and saved,
so nothing is silently dropped. SYSTEMIC failures still fail loud (listing 404
handling, the decodability guard, a Sabah date/URL mismatch). If `n_skipped` is
large for one source (e.g. Cumhuriyet's consent gate under volume), that's the
signal to switch that source to `--playwright`.
